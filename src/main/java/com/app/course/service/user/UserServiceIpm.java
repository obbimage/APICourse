...
public class UserServiceIpm implements UserService {
    ...
    public ResponseEntity<RepositoryObject> insertUser(User user) {
        ...
        return users == null ? ResponseEntity.status(HttpStatus.OK).body(new RepositoryObject("ok", "Đăng ký thành công", userRepository.save(user))) :
                // status 409 -> xảy ra sung đột
                ResponseEntity.status(HttpStatus.CONFLICT).body(new RepositoryObject("failed", "Tài khoản đã tồn tại ", ""));
...
//       tim user
    ...
    @Override
    public ResponseEntity<RepositoryObject> getAllEducatorToUser(int pageNumber, int pageSize) {
        Pageable pageable = PageRequest.of(pageNumber,pageSize);
        String role = Constants.ROLE_EDUCATOR;
        var educators = userRepository.findByRole(role, pageable);
        return Response.resultOk(educators);
    }

    @Override
    public ResponseEntity<RepositoryObject> findEducators( String keyword,int pageNumber,int pageSize) {
        String role = Constants.ROLE_EDUCATOR;
        Pageable pageable = PageRequest.of(pageNumber,pageSize);
        Long id = null;
        try{
            id = Long.parseLong(keyword);
            var educators = userRepository.findById(id,pageable);
            return Response.resultOk(educators);
